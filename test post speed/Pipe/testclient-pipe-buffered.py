...
if __name__ == "__main__":
	...
	while(True):
		...
		try:
			...
			buf[0].append(int(vals[0]))
			buf[1].append(int(vals[1]))
			buf[2].append(float(vals[2]))
			buf[3].append(float(vals[3]))
			buf[4].append(float(vals[4]))
			buf[5].append(float(vals[5]))
			buf[6].append(float(vals[6]))	
			
			counter += 1

			if(counter >= 200):
				line = {'image_name': buf[0][currNum:currNum + counter] , 'CLASS_ID': buf[1][currNum:currNum + counter] , 'confidence' : buf[2][currNum:currNum + counter] , 'xmin' : buf[3][currNum:currNum + counter] , 'ymin' : buf[4][currNum:currNum + counter], 'xmax' : buf[5][currNum:currNum + counter] , 'ymax' : buf[6][currNum:currNum + counter]}
				if post_result(token,line)==0:        # If post_result failed, exit.
					print "Posting Result Failed, Exiting, Bye!"
					sys.exit()
				else:
