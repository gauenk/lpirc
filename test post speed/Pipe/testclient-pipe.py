...
if __name__ == "__main__":
	...
	while(True):
		...
		try:
			...
			line = {'image_name': buf[0][currNum] , 'CLASS_ID': buf[1][currNum] , 'confidence' : buf[2][currNum] , 'xmin' : buf[3][currNum] , 'ymin' : buf[4][currNum], 'xmax' : buf[5][currNum] , 'ymax' : buf[6][currNum]}
			if post_result(token,line)==0:        # If post_result failed, exit.
				print "Posting Result Failed, Exiting, Bye!"
				sys.exit()
			else:
