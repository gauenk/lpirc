...
if __name__ == "__main__":
	...
	print "simulating processing and writing"
	while(counter < 1000):
		simulate_work(300000)
		buf[0].append(counter)
		buf[1].append(100)
	...
	line = {'image_name': buf[0][0 : counter] , 'CLASS_ID': buf[1][0 : counter] , 'confidence' : buf[2][0 : counter] , 'xmin' : buf[3][0 : counter] , 'ymin' : buf[4][0 : counter], 'xmax' : buf[5][0 : counter] , 'ymax' : buf[6][0 : counter]}
	if post_result(token,line)==0:        # If post_result failed, exit.
		print "Posting Result Failed, Exiting, Bye!"
		sys.exit()
