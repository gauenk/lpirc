...
def voc_eval(det_lines,
    ...
    confidence = np.array([round(float(x[2]),3) for x in det_lines])
    BB = np.array([[round(float(z),1)+1 for z in x[3:]] for x in det_lines])
...
def _calc_map_(det_lines,year='2007'):
    ...
    #dataset = "imagenet"
    #devkit_path = "/home/gauenk/Documents/data/ilsvrc/ILSVRC/"
    ...
    for i in range(1,len(det_lines)):
        ...
        rec, prec, ap = voc_eval(
            det_lines[i], annopath, imagesetfile, cls, cachedir, ovthresh=0.5)
        aps += [ap]
